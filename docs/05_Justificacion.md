# ms-delivery — Justificación del servicio y cobertura de requisitos

**Caso caso05 — FoodGo** (Delivery de comida) · EP01 JVY0101

Este documento justifica la existencia de **ms-delivery** como microservicio independiente: qué requisitos del negocio cubre (funcionales, no funcionales y de seguridad), por qué está delimitado así (SRP), y qué tecnología AWS se usa para cada responsabilidad y **por qué**. Los diagramas que respaldan esta justificación están en `docs/diagramas/`.

---

## 1. Misión del servicio

ms-delivery mantiente el seguimiento en tiempo real del proceso: consume los eventos y señales del dominio (estados, posición, sensores, etapas), mantiene el estado vigente consultable en milisegundos y notifica las transiciones del caso caso05 (FoodGo).
...
| RF | Requisito (de `00_PresentacionEmpresa.md`) | Qué hace ms-delivery al respecto | Evidencia |
|----|------------------------------------------|-------------------------------|-----------|
| **RF-05** | Asignar repartidor y mostrar seguimiento GPS del pedido en tiempo real | Ingresa y consulta el estado en tiempo real: cada evento actualiza el recurso y dispara la notificación correspondiente | diagrama de secuencia (evento en tiempo real) |
...
| RNF | Criterio | Cómo lo cumple ms-delivery | Decisión técnica |
|-----|----------|--------------------------|------------------|
...
| **RNF-01** (Escalabilidad) | Absorber picos de pedidos (almuerzo/cena, fines de semana) escalando pedidos y pagos por separado | Auto scaling independiente de este servicio (2→20 tareas Fargate según carga) | ECS Fargate + alarmas de CloudWatch: solo este componente escala en el pico |
| **RNF-06** (Consistencia) | Los pedidos no deben perderse ante fallas (persistencia + colas); el pago debe ser atómico con el pedido | Escritura persistida antes de confirmar; eventos por cola con DLQ ante fallas | Aurora transaccional + SQS persistido + idempotencia |

**Justificación SRP (IE9):** ms-delivery tiene **una sola razón de cambio**: las reglas de seguimiento y transición de estados del proceso en tiempo real. Si mañana cambia esa regla, **ningún otro servicio se modifica**.
...
| Tecnología | Para qué se usa en ms-delivery |
|------------|------------------------------|
...
| Servicio AWS | Rol en ms-delivery | Por qué se eligió |
|--------------|----------------|--------------------|
...
## 6. Delimitación: qué NO hace ms-delivery (IE9/IE10)
...
| restaurantes | ms-restaurantes | razones de cambio distintas: el catálogo consulta y publica; las operaciones de negocio las orquesta el servicio transaccional |
...
├── secuencia/
│   └── Secuencia-Entrega   evento en tiempo real
└── infraestructura/
