# Delivery — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `EntregaApplication` | Punto de entrada Spring Boot |
| `controller/EntregaController.java` | API REST expuesta en `/api/entregas` |
| `service/EntregaService.java` | Lógica de negocio y transacciones |
| `repository/EntregaRepository.java` | Acceso JPA a H2 |
| `model/Entrega.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → EntregaController → EntregaService → EntregaRepository → H2`
