package cl.duoc.jv0101.caso05.delivery.cucumber;
...
public class EntregaSteps {
    ...
    private Long id;
    private ResponseEntity<Entrega> respuesta;
    private ResponseEntity<Void> respuestaVoid;

    private String url() {
        return "http://localhost:" + port + "/api/entregas";
    }
    ...
    public void servicioDisponible(String servicio) {
        ResponseEntity<Entrega[]> listado = rest.getForEntity(url(), Entrega[].class);
    ...
    public void crearRecurso(String recurso, String nombre) {
        ResponseEntity<Entrega> creado = rest.postForEntity(
                url(), body(nombre), Entrega.class);
    ...
    public void consultarCreado(String recurso) {
        respuesta = rest.getForEntity(url() + "/" + id, Entrega.class);
    }
    ...
    public void actualizar(String recurso, String nombre) {
        respuesta = rest.exchange(url() + "/" + id, HttpMethod.PUT, body(nombre), Entrega.class);
    }
    ...
    public void consultarEliminado(String recurso, int codigo) {
        ResponseEntity<Entrega> noExiste = rest.getForEntity(url() + "/" + id, Entrega.class);
